...
let selectedAddress = null   // MAC address or unique ID of the currently selected device. Used when reconnecting to find the device again.
...
function findUARTCharacteristics(services) {
  ...
  services.forEach((s, serviceId) => {
    s.characteristics.forEach((ch, charId) => {
      ...
      if (ch.uuid === '6e400002b5a3f393e0a9e50e24dcca9e') {
        ...
        uartRx.on('data', (data) => {
          if (mainWindow !== null) {
            mainWindow.webContents.send('uartRx', data)
          }
        })
        uartRx.notify(true)
      }
...
app.on('ready', () => {
  ...
  if (os.platform() === 'linux' && !runningAsRoot()) {
    ...
    dialog.showErrorBox('Bluetron', 'WARNING: This program should be run as a root user with sudo!')
  }
  ...
  mainWindow = new BrowserWindow({
    ...
    height: 800,
    title: 'Bluetron',
    icon: __dirname + '/../assets/ble_icon.png'
  })
  ...
  ipcMain.on('readChar', (event, serviceId, charId) => {
    ...
    if (selectedIndex !== null) {
      ...
      device.services[serviceId].characteristics[charId].read((error, data) => {
        ...
        if (error) {
          ...
          mainWindow.webContents.send('charUpdated', serviceId, charId, data)
        }
      })
    }
  })
