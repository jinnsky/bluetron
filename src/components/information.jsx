...
class Information extends React.Component {
  ...
  componentDidMount() {
    ...
    let service = ipcRenderer.sendSync('getServices', this.props.params.index)
    this.setState({device: device, services: service})
 
    // Setup services updated event to receive service/characteristic changes.
    ipcRenderer.on('charUpdated', this.charUpdated)
  }

  componentWillUnmount() {
    // Be careful to make sure state changes aren't triggered by turning off listeners.
    ipcRenderer.removeListener('charUpdated', this.charUpdated)
  }

  charUpdated(event, serviceId, charId, value) {
    // Convert to string if necessary
    if (!(serviceId === 3 && charId === 0)) {
      value = String(value)
    }
  ...
  readChar(serviceId, charId) {
    ...
    ipcRenderer.send('readChar', serviceId, charId)
  }
